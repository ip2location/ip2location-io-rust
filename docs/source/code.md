...
:param string language: (Required) Translation information(ISO639-1). The translation is only applicable for continent, country, region and city name.
:::{note}
Note: This parameter is only available for Plus and Security plan only. Set the value to None to bypass the parameter.
:::
:return: Returns the geolocation information in Struct. Refer below table for the fields avaliable in the Struct
:rtype: Struct
...
|as_name|string|Autonomous system (AS) name.|
...
|fraud_score|int|Potential risk score (0 - 99) associated with IP address. A higher IP2Proxy Fraud Score indicates a greater likelihood of fraudulent activity and a lower reputation.|
...
:return: IP2WHOIS Domain WHOIS result in Struct. Refer below table for the fields avaliable in the Struct.
:rtype: Struct
...
```{py:function} get_punycode(domain)
...
```{py:function} get_normaltext(domain)
Get Normal Text.
...
```{py:function} get_domainname(url)
...
```{py:function} get_domainextension(str)
...
:::
:return: Returns the domain list in Struct. Refer below table for the fields avaliable in the Struct
:rtype: Struct
